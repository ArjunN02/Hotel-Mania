...
const SingleRoom = () => {
  ...
  useEffect(() => {
    const getRoomDetails = async () => {
      try {
          const data = await axios.post(`${process.env.REACT_APP_API_URL}/api/room/details`,{ id }).then((response) => {
        // console.log(response.data.roomDetails);
    ...
    const getHotelDetails = async () => {
      try {
          const data = await axios.post(`${process.env.REACT_APP_API_URL}/api/hotels/singleHotel`,{ hotelId }).then((response) => {
          ...
          <Col md={{ span: 18 }} xs={{ span: 24 }}>
            <div>
              ...
              <h5 style={{ color: "#627e89", fontFamily: "cursive" }}>
                Book a stay over Rs.1500 at this property and get a free airport
                taxi.
              </h5>
            </div>
          </Col>
